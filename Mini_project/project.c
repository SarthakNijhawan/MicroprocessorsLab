...
int milli=0,temp=0,j=0,final=0,INCREMENT=0;
...
unsigned int DT=35;
sbit PIN = P1^2;		// This is to check the mode of the Temperature Controller
	...
	while(1)
	{
		if(PIN == 1){
		   ...
		   for(j;j<10;j++)
		   {
				...
				final = (final*j+adcVal)*1.0/(j+1);	// Running Average
			}

			milli = final*50.0/1024 ;	//temperature
			   
		   if(milli <= 50){
				milli = 35;
			}
		   else if(50<milli && milli<=150){
				milli = 45;
			}
	       	   else if(150<milli && milli<=250){
				milli = 55;
			}
		   else if(250<milli && milli<=350){
				milli = 65;
			}
		   else if(350<milli && milli<=450){
				milli = 75;
			}
		   else{
				milli = 85;
			...
			DT = milli;		// Desired Temperature

			// LCD_Ready();
			LCD_CmdWrite(0xC0);
			temp=milli;
			temp=temp/10;	// Temperature
			...
			final = 0;		// Running avg
			j=0;			// loop var
			PIN = 1;		// Setting the pin back to input mode
	     }
		else{
			LED=PIN;
			for(j;j<10;j++)
			{
				...
				adcVal = (data_save_high <<8) + (data_save_low);
				final = (final*j+adcVal)*1.0/(j+1);
			}

            milli = final*50.0/1024 ;
